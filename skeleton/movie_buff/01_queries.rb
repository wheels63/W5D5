...
def harrison_ford
  ...
  #
  # It's possible to join based on active record relations 
  # defined in models.
  #
  # Find the id and title of all movies in which 
  # Harrison Ford appeared but not as a lead actor
 
  # find the movie and title of all movies where HF is not the lead actor
  # associate movie with actor
  # start with movie, join actors, 
  ...
  #   .joins(:actors)
  #   .select(:id, :title)
  #   .where(actors: { name: 'Harison Ford' })
    #   .where(movies: { title: 'Blade Runner' })
